...
- “三枚铜钱”二级页已在本地实现：同页完成事项分类、问题输入、六次三钱记录与结果展开，复用现有二级页昼夜主题并保留输入上下文
...
- 三枚铜钱单页工具流程正在 `agent/three-coin-workspace` 分支验证，尚未获得公网发布确认。
...
5. 验证 Issue #25 的三枚铜钱单页交互后，将同一页面骨架逐项复用到其他推荐工具。
6. 审核 Issue #19 的五个灵签信二级工具，并在确认后决定 AI 模型接入与全国地区数据范围。
7. 按 `docs/ROADMAP.md` 通过 GitHub Issues 推进其他二级页面，而不扩大当前一级首页范围。
